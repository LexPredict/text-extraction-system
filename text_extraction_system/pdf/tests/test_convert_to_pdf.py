...
def test_xlsx():
    def assert_pdf(fn: str):
        ...
        assert txt.count(b'fitting') == 144  # just counted them in the original xlsx
...
def test_docx():
    def assert_pdf(fn: str):
        txt = process(fn)
        assert txt.count(b'document') == 105
...
def test_odf():
    def assert_pdf(fn: str):
        txt = process(fn)
        assert txt.count(b'document') == 110
        with pikepdf.open(fn) as pdf:
            assert len(pdf.pages) == 3
