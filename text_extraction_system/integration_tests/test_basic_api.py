...
class TestBasicAPICallbackMethods(unittest.TestCase):
    ...
    def test_basic_api_call_back_tables_msgpack(self):
        ...
        request_id = self.client.schedule_data_extraction_task(fn, doc_language='en', call_back_url=self.call_back_url,
                                                               call_back_additional_info='hello world',
                                                               output_format=OutputFormat.msgpack)
        self.srv.wait_for_test_results(timeout_sec=60, assert_func=assert_func, assert_func_args=[request_id])
    ...
    def test_basic_api_call_back_cancel(self):
        ...
        try:
            self.srv.wait_for_test_results(timeout_sec=15, assert_func=assert_func, assert_func_args=[])
        except TimeoutError:
            pass
