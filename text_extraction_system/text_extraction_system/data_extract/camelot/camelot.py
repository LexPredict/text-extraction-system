...
from text_extraction_system.pdf.pdf import iterate_pages, extract_full_images_from_pdf


class CustomizedLattice(Lattice):
    def _generate_image(self):
        # We don't need to additionally generate the image here as it is already generated for the OCR needs.
...
def extract_tables(pageno: int, page_layout: LTPage, pdf_page_image_fn: str,
                   table_parser: TableParser = TableParser.lattice, min_accuracy: int = 60) -> List[CamelotTable]:
    ...
    # putting a dummy file name to avoid Camelot arguing. It extracts the page number from the file name.
    try:
...
def get_extractor(pdf_page_image_fn: str, table_parser: TableParser = TableParser.lattice):
    areas = None
    detect_areas = table_parser == TableParser.area_stream or table_parser == TableParser.area_lattice
    extract_method = 'lattice' if table_parser in (TableParser.lattice, TableParser.area_lattice) else 'stream'
    if detect_areas:
...
def extract_tables_from_pdf_file(pdf_fn: str, pdfminer_advanced_detection: bool = False,
                                 table_parser: TableParser = TableParser.lattice,
                                 min_accuracy: int = 60) -> List[CamelotTable]:
    res: List[CamelotTable] = list()
    page_num = 0
    with extract_full_images_from_pdf(pdf_fn) as image_fns:
