...
class ImageAberrationDetection:
    ...
    def determine_skew_dilated_rows(cls, image_fn: str) -> Optional[float]:
        ...
        scale_k = math.pow(1.0 * (cls.SKEW_IMAGE_DETECT_TARGET_SIZE[0] *
                                  cls.SKEW_IMAGE_DETECT_TARGET_SIZE[1]) / img_area, 0.5)
        new_min_size = int(min_size * scale_k)
        new_min_size = max(new_min_size, min(min_size, cls.MIN_IMAGE_DIMENSION))
        ...
        try:
            ...
            blur = cv2.GaussianBlur(gray, (cls.IMAGE_BLUR_RADIUS, cls.IMAGE_BLUR_RADIUS), 0)
